...
public class NotaMapper {
    ...
    public Nota toNota(NotaUpdateRequest notaRequest) {
        Usuario dono = usuarioRepository.findById(notaRequest.donoId())
                .orElseThrow(() -> new RuntimeException("Usuário donoId não encontrado!"));
        ...
        return Nota.builder()
                ...
                .dataCriacao(dono.getDataCriacao())
                .dono(dono)
    ...
    public Nota toNota(NotaRequest notaRequest) {
        Usuario dono = usuarioRepository.findById(notaRequest.donoId())
                .orElseThrow(() -> new RuntimeException("Usuário donoId não encontrado!"));

        List<Usuario> usuariosCompartilhados = Collections.emptyList();
        if (notaRequest.usuariosCompartilhadosIds() != null && !notaRequest.usuariosCompartilhadosIds().isEmpty()) {
